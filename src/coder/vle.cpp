...
msb(numsamples),maxbpn(maxbpn),numsamples(numsamples)
{
  ...
  bpn=0;
    /*for (int i=0;i<32;i++) {
         double p0=1./(1+pow(2,(i+1)/16.));
         double p1=1./(1+pow(2,(i+1)/32.));
         gc0[i]=clamp((int)round(p0*PSCALE),1,PSCALEm);
         gc1[i]=clamp((int)round(p1*PSCALE),1,PSCALEm);
      }*/
}
...
int BitplaneCoder::PredictRef()
{
  ...
  std::vector <int>vp={pc1->p1,pc2->p1,pc3->p1};
...
int BitplaneCoder::PredictSig()
{
  ...
  std::vector <int>vp={pc1->p1,pc2->p1};
...
int BitplaneCoder::PredictSSE(int p1)
{
  ...
  psse=&sse[ssectx];
  std::vector <int>vp={psse->Predict(p1),p1};
  return finalmix[0].Predict(vp);
}
